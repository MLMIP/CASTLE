...
def split_data(data, k=5):
    # 首先，我们根据前缀名的第一个'_'前面的字符对数据进行分组
    groups = {}
    for item in data:
        if item.split('_')[-1]=='fu':
            prefix = item.split('_')[-1]
    ...
    for group in groups.values():
        ...
        for i in range(k):
            start = i * size
            end = start + size if i != k - 1 else len(group)
            result[i].extend(group[start:end])
    return result
...
def main():
    ...
    parser.add_argument('-id', '--data-index', dest='data_index', type=str,default='Data/alltrain_data/data_index/Mix.pkl',
                        required=False, help='The file stores information about the samples used for five-fold cross-validation.')
    parser.add_argument('-il', '--data-label-index', dest='data_label_index', type=str,default='Data/alltrain_data/data_label_index/Mix_label.pkl',
                        required=False,help='The file stores label information about the samples used for five-fold cross-validation.')
    parser.add_argument('-or', '--outputdir-result', dest='outputdir_result', type=str,default='Result/all_indicator/fiveFold_crossValidation_Mix.xlsx',
                        required=False, help='The folder of the Zn_indicator.')
    parser.add_argument('-of', '--outputdir-file', dest='outputdir_file', type=str,default='Result/all_file/Mix',
    ...
    # for t in all_label:
    #     if t==1: x+=1
    # xishu = x/(len(all_label)-x)
    num_zhe = 0
    fold_num = 0
    data_5 = split_data(all_pdb_tag)

    for i in range(5):
        pdb_tag_train = [x for j, X_fold in enumerate(data_5) if j != i for x in X_fold]
        pdb_tag_test = data_5[i]
        fold_num += 1
        model = Model_Net()
        model.to(devices)
        ...
        random.shuffle(pdb_tag_train)
        random.shuffle(pdb_tag_test)
        ...
        best_model_wts = None
        data_train_5 = split_data(pdb_tag_train)
        for j in range(5):
            pdb_tag_train = [x for j, X_fold in enumerate(data_train_5) if j != i for x in X_fold]
            pdb_tag_valid = data_train_5[i]
            random.shuffle(pdb_tag_train)
            random.shuffle(pdb_tag_valid)
