...
def remove_matching_elements(a, b, indices):
    # 将数组a和b中的元素都用'_'分割
    a = [item.split('_') for item in a]
    b = [item.split('_') for item in b]
    # 创建一个新的数组来存储结果
    new_a = []
    # 遍历数组a中的每个元素
    for item_a in a:
        # 检查该元素是否在数组b中
        if not any(all(item_a[index] == item_b[index] for index in indices) for item_b in b):
            # 如果没有匹配的元素，则将该元素添加到新的数组中
            new_a.append('_'.join(item_a))
    # 返回新的数组
    return new_a
...
def main():
    ...
    parser.add_argument('-or', '--outputdir-result', dest='outputdir_result', type=str,default='Result/all_indicator/Test/independent_test.xlsx',
                        required=False, help='The folder of the Zn_indicator.')
    parser.add_argument('-of', '--outputdir-file', dest='outputdir_file', type=str,default='Result/all_file/Test/independent_test',
                        required=False, help='The folder of the Zn_result_file.')
    parser.add_argument('-dw', '--dir-weight', dest='dir_weight', type=str, default='Result/Weight/gnn2_xishu_0822_42/',
                        required=False, help='The folder of the Zn_result_file.')
    ...
    if not os.path.exists(dir_weight):
    ...
    with open('Data/test_data/data_index/Test.pkl', 'rb') as f:
        pdb_tag_test = pickle.load(f)
    indices = [0, 1, 3, 4, 6]
    pdb_tag_train = remove_matching_elements(all_pdb_tag, pdb_tag_test, indices)

    x = 0
    pdb_label_train = []
    for t in pdb_tag_train:
        if t.split('_')[-1] == 'fu':
            x += 1
            pdb_label_train.append(0)
        else:
    ...
    for train_inx, valid_inx in cv.split(pdb_tag_train, pdb_label_train):
        fold_num += 1
        model = Model_Net()
        model.to(devices)
        best_model_wts = None
        ...
        df.to_excel(outputdir_file + '{}_cishu.xlsx'.format(fold_num), index=False)
