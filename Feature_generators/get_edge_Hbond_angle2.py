...
class get_edge(object):
    ...
    def get_mfs_redisue_neighbor_index(self, residue_dm,metal_pos, threshold1=3, threshold2=5):
        source, target, distance,ligand1,ligand2 = [], [], [], [], []
        for i in range(residue_dm.shape[0]):
            if i!=metal_pos and residue_dm[metal_pos, i] <= threshold1:
                source.append(metal_pos)
                target.append(i)
                distance.append(residue_dm[metal_pos, i])
                ligand1.append(i)
        for lig1_pos in ligand1:
            for i in range(residue_dm.shape[0]):
                if i not in ligand1 and i!=metal_pos and residue_dm[lig1_pos, i] <= threshold2:
    ...
    def generate_mfs_residue_edge_feature(self ,pdb_array,pdb_tag,pdbdir_path,distance_type='mfs'):
        residue_index, pdb_pos_dict = get_edge().get_residue_info(pdb_array)
        residue_dm,metal_pos  = get_edge().get_residue_distance_matrix(pdb_array, residue_index, distance_type)
        #每个氨基酸残基到其他残基的距离排序
        source, target, distance = get_edge().get_mfs_redisue_neighbor_index(residue_dm,metal_pos,3,5)
        ...
        residue_dm, metal_pos = get_edge().get_residue_distance_matrix(new_pdb_array, residue_index, distance_type)
        # 每个氨基酸残基到其他残基的距离排序
        source, target, distance = get_edge().get_mfs_redisue_neighbor_index(residue_dm, metal_pos, 3, 5)
