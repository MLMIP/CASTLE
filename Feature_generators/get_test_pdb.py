...
def get_pipeipdb_info(pdbname, pos, from_acid, to_acid , label):
    global set_to_acid
    RemoveDir('../Data/demo')
    tag = False
    ...
    for i in range(1, 50):
        ...
        # pdbfile='Data/demo/1buv_1.site.pdb
        ...
        if flag1 == True and flag == True:
            ...
            if not os.path.exists(folder_from+'/{}_{}_{}_{}_{}_{}_{}_unseen.pdb'.format(metal,pdbname.lower(),i,pos,from_acid,chain_name,to_acid)):
              os.rename(pdb,folder_from+'/{}_{}_{}_{}_{}_{}_{}_unseen.pdb'.format(metal,pdbname.lower(),i,pos,from_acid,chain_name,to_acid))
            end_pdb = folder_from+'/{}_{}_{}_{}_{}_{}_{}_unseen.pdb'.format(metal,pdbname.lower(),i,pos,from_acid,chain_name,to_acid)
            tag = True
            ...
            torch.save(y, labelpath_filder + '/{}_{}_{}_{}_{}_{}_{}_unseen.pt'.format(metal, pdbname.lower(), i, pos, from_acid,chain_name, to_acid))
            torch.save(metal_fea, metal_filder + '/{}_{}_{}_{}_{}_{}_{}_unseen.pt'.format(metal, pdbname.lower(), i, pos, from_acid, chain_name,to_acid))

            all_pdb_tag.append('{}_{}_{}_{}_{}_{}_{}_unseen'.format(metal, pdbname.lower(),i,pos,from_acid,chain_name,to_acid))
            ...
            try:
                if (PC.ProteinCheck(ProteinSequence) > 0):
                    ...
                    torch.save(seqfeatures,seq_from_filder + '/{}_{}_{}_{}_{}_{}_{}_unseen.pt'.format(metal, pdbname.lower(), i, pos, from_acid,chain_name, to_acid))

                    seqfeatures_to = np.array(seqfeatures_to).astype(float)
                    seqfeatures_to = torch.tensor(seqfeatures_to, dtype=torch.float)
                    torch.save(seqfeatures_to,seq_to_filder + '/{}_{}_{}_{}_{}_{}_{}_unseen.pt'.format(metal, pdbname.lower(), i, pos, from_acid,chain_name, to_acid))
    ...
    if not tag:
        print("未找到pdb:{},metal:{},pos:{},acid:{}样本对应的氨基酸".format(pdbname, metal, pos, from_acid))
        with open(error_examples_file, 'a') as f:
            f.write("未找到pdb:{},metal:{},pos:{},acid:{}样本对应的氨基酸".format(pdbname, metal, pos, from_acid) + '\n')
    return
...
def get_excel_data(file):
    ...
    return pdbname.values, location.values, orig_acid.values, to_acid.values, label.values


def get_metal_seq_info(excel_add):
    pdbname, location, orig_acid, to_acid, label = get_excel_data(excel_add)
    for i in range(0, len(pdbname)):
        if location[i][0] == 'No_1': continue
        get_pipeipdb_info(pdbname[i][0], location[i][0], orig_acid[i][0], to_acid[i][0],label[i][0])
...
def get_filename_prefixes(folder_path):
    # 初始化一个空列表来保存文件名前缀
    prefixes = []
...
if __name__ == '__main__':
    ...
    folder_from = '../Data/test_data/metalpdb'
    seq_from_filder = "../Data/test_data/seq_fea"
    seq_to_filder = "../Data/test_data/seq_to_fea"
    metal_filder = "../Data/test_data/metal_fea"
    labelpath_filder = "../Data/test_data/label"
    data_path = '../Data/test_data/record_data.txt'
    error_examples_file = "../Data/test_data/nofind_metalpdb.txt"
    ...
    all_pdb_tag = []
    get_metal_seq_info('../Data/excel/test.xlsx')
