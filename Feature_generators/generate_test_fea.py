...
def feature_alignment1_muchatom_res60(pdb_tag,selected_res, selected_atom, feature_data, res_dict, atom_dict, mut_pos,metal_pos,node_pos_dict,atom_pos_dict,pdb_path):
    ...
    try:
        ...
        with open("../Data/test_data/allatom_fea_error.txt", "a") as f:
            f.write(f"{pdb_tag}\n")
...
def generate_input(pdb2mutation, outfile):
    ...
    with open(outfile, 'w') as f_r:
        for pdb_flag in pdb2mutation:
            ...
            all_pdb.append(pdb_flag)
            f_r.write(metal_type + '\t' + pdb_id + '\t' + pdb_i + '\t' + chain_id + '\t' + mut_pos + '\t' + wild_aa + '/' + mut_aa +  '\n')

    print("总共生成{}条数据".format(len(all_pdb)))
    for x in pdb2mutation:
        if x not in all_pdb:
            with open('Data/test_data/nogetfea_samples.txt','a') as f:
                f.write(x)
                f.write('\n')
...
metalpdb_from = '../Data/test_data/'+'metalpdb'
label_filder = '../Data/test_data/'+"label"
...
data_path = '../Data/test_data/record_data.txt'
dataout_path = '../Data/test_data/get_record_data.txt'
