...
import shutil
import os
from Feature_generators.calc_sasa import SASA
from Feature_generators.calc_ss import Secondstructure
from Feature_generators.calc_depth import calc_depth_noclean
from Feature_generators.get_edge_Hbond_angle2 import get_edge
from Feature_generators.calc_atom_fea import  get_atom_fea
import numpy as np
import torch
...
def feature_alignment1_muchatom_res60(pdb_tag,selected_res, selected_atom, feature_data, res_dict, atom_dict, mut_pos,metal_pos,node_pos_dict,atom_pos_dict,pdb_path):
    ...
    try:
        ...
        with open("predict_save_data/allatom_fea_error.txt", "a") as f:
            f.write(f"{pdb_tag}\n")
    ...
    for pos in selected_res:
        ...
        # res_feature_dict[pos] = aa_code + ss + depth + properties + sasa + pssm + list(hhm)  + is_mut
...
def get_pdb_array(pdb_file):
    ...
    with open(pdb_file, 'r') as pdbfile:
        ...
        for i,item in enumerate(pdb_array):
            # if item[0] == 'HETAT' and item[-1] in metal_List:
            if item[0] == 'HETAT' and item[-1] in metal_List:
                metal_pos_dict[i] = item[6]
                metal_list.append(item)
        end_metal_pos = list(metal_pos_dict.values())[0]
        for metal_pos in metal_pos_dict.values():
            temp = []
            for name, grade in metal_pos_dict.items():
                if grade == metal_pos:
                    temp.append(name)
            for x in temp:
                pdb_array[x][6] = end_metal_pos
        pdb_array = np.delete(pdb_array, list(metal_pos_dict.keys()), 0)
        pdb_array = np.concatenate((pdb_array,metal_list),axis=0)
    return pdb_array
...
def get_nearest_mfs_resindex_noH(pdb_file):
    selectes_pdb_pos = []
    atompos2index = {}
    new_pdb_array = []
    ...
    residue_dm,metal_pos = get_edge().get_residue_distance_matrix(pdb_array, residue_index, 'mfs')
    index = residue_dm.argsort()[metal_pos, :]
    ...
    for atom_info in pdb_array:
        if atom_info[2][0] == 'C' or atom_info[2][0] == 'N' or atom_info[2][0] == 'O' or atom_info[2][0] == 'S' or atom_info[-1] in metal_List :
            if atom_info[6] in selectes_pdb_pos :
                atompos2index[atom_info[1]] = index
                new_pdb_array.append(list(atom_info))
                index += 1
            if atom_info[-1] in metal_List :
                metal_pos_inpdb = atom_info[6]
        # if len(metal_array) >1:
...
def get_res_atom_dict(pdb_array):
    ...
    for atom_info in pdb_array:
        try:
            ...
            try:
                # aa = three_to_one(NON_STANDARD_SUBSTITUTIONS[atom_info[4]])
                aa = atom_info[4]
            except:
                continue
...
def generate_wt_features(pdb_flag, metalpdb_from):
    pdb_file = f'{metalpdb_from}/{pdb_flag}.pdb'
    ...
    try:
        ...
        with open('predict_save_data/depth_error.txt', 'a') as f:
...
def generate_predict_input(pdb2mutation):
    outpdbtag_list = []
    save_dir = 'predict_save_data/'
    ...
    node_pos_from_filder = save_dir + "node_pos_fea"
    basic_attn_peiwei_from_filder = save_dir + "basic_attn_peiwei"
    metalpdb_from = save_dir + 'metalpdb'

    if not os.path.exists(res_node_from_filder):os.makedirs(res_node_from_filder)
    if not os.path.exists(res_edge_from_filder):os.makedirs(res_edge_from_filder)
    if not os.path.exists(res_index_from_filder):os.makedirs(res_index_from_filder)
    if not os.path.exists(atom_node_from_filder):os.makedirs(atom_node_from_filder)
    if not os.path.exists(atom_edge_from_filder):os.makedirs(atom_edge_from_filder)
    if not os.path.exists(atom_index_from_filder):os.makedirs(atom_index_from_filder)
    if not os.path.exists(atom2res_from_filder):os.makedirs(atom2res_from_filder)
    if not os.path.exists(node_pos_from_filder):os.makedirs(node_pos_from_filder)
    if not os.path.exists(basic_attn_peiwei_from_filder): os.makedirs(basic_attn_peiwei_from_filder)
    if not os.path.exists(metalpdb_from): os.makedirs(metalpdb_from)

    data_path = 'predict_save_data/record_predict_data.txt'
    ...
    with open(data_path, 'r') as file:
        ...
        for line in file:
            # 去除行尾的换行符
            pdb_file = line.strip()
            if pdb_file == '' or os.path.exists(f'{metalpdb_from}/{pdb_file}.hb2'):
                continue
            # 构造HBPLUS命令 按照官网进行HBPLUS的安装和路径修改
            file_path = os.path.abspath(f'{metalpdb_from}/{pdb_file}.pdb')
            command = f'/media/s3/wjf/biology-data/hbplus/hbplus {file_path}'
            # 在命令行中执行命令
            os.system(command)
            # 移动文件
            shutil.move(f'{pdb_file}.hb2', metalpdb_from)
    for pdb_flag in pdb2mutation:

        pdb_file_wt = f'{metalpdb_from}/{pdb_flag}.pdb'
        metal_type = pdb_flag.split('_')[0]
        ...
        try:
            data_wt = generate_wt_features(pdb_flag,metalpdb_from)
        except OSError:
            continue

        ############################# generating wild input #####################################
        try:
            _, _, pdb_array_wt,metal_pos =  get_nearest_mfs_resindex_noH(pdb_file_wt)
        except:
            print("错误样本为为为为：{}".format(pdb_flag))
            continue
        if metal_pos=='-1':
            print("错误样本：{}".format(pdb_flag))
            continue
        try:
            pdb_array_wt,res_edge_index_wt, res_edge_feature_wt, atom_res_index_wt,node_pos,res_index_pos_dict_wt, atom_index_pos_wt,node_pos_dict,atom_pos_dict,basic_attn,basic_attn_peiwei = get_edge().generate_mfs_residue_edge_feature(pdb_array_wt, pdb_flag,metalpdb_from)
        except:
            continue
        ...
        res_feat_dict_wt, atom_feat_dict_wt = feature_alignment1_muchatom_res60(pdb_flag,selected_res_wt, selected_atom_wt,
                                                                data_wt, res_dict_wt, atom_dict_wt,
                                                                mut_pos, metal_pos,node_pos_dict,atom_pos_dict,metalpdb_from)
        ...
        outpdbtag_list.append(pdb_flag)

    print("总共生成{}条数据".format(len(outpdbtag_list)))
    return outpdbtag_list
